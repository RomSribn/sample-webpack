...
interface ApplicationSelectorProps {
  ...
  onChange: (appName: { key: string, version: string, url: string, name: string } | undefined) => void;
}

export function RemotesSelector({ remoteKey, version, remote, onChange: onRemoteChange }: ApplicationSelectorProps) {
  ...
  const onChange = (event: unknown, newValue: string) => {
    ...
    const _remote = remote?.versions
      .find((version) => version.name === newValue);
    if (!_remote) return;
