import {
  addDisabledLay,
  createNewDataList,
  generateConfigForHtml,
  getDeployedBadge,
  getFormContent,
  getInputFieldHTML,
  setActionButtons
} from './content-page/utils';
import {
  IAppLevel,
  IMapedData,
  IRemotesLevel,
  ITagLevel,
  IVersionLevel
} from '../../../../zephyr-side-panel/src/app/interfaces/interfaces';
import { getAppData, IAppResponseData, publishAppVersion } from '../../../../zephyr-side-panel/src/app/mocks/api';
import { setActionButtonsEventListeners } from './content-page/clientEvents';
import { ICurrentValue, session } from '../../../../zephyr-side-panel/src/app/storage/session';
...
export class ContentClass extends HTMLElement {
  ...
  private static currentValues: ICurrentValue = {
    appLevel: null,
    tagLevel: null,
    versionLevel: null,
    remoteLevel: null
  };
  private static deployedVersion: ICurrentValue = {
    appLevel: null,
    tagLevel: null,
    versionLevel: null,
    remoteLevel: null
  ...
  public static async checkCallBack(val: {
    fieldList: string;
    controller: string;
    ...
    if (val.fieldList === 'appLevel') {
      if (this.currentValues.appLevel !== val.value.name) {
        ...
        await session.setCurrentFormValues(this.currentValues);
        this.initForm();
        this.getMainContent();
  ...
  private static mapAppLevel(): IAppLevel {
    ...
    if (!this.currentValues.appLevel) {
      ...
      session.setCurrentFormValues(this.currentValues);
    }
  ...
  static getAppLevelHTML(): string {
    ...
    return getInputFieldHTML(data.fieldListName, 'Name', generateConfigForHtml(data));
  }

  static getTagLevelHTML(): string {
    const data = this.mapedObjectData.tagLevel;
    return getInputFieldHTML(data.fieldListName, 'Tag', generateConfigForHtml(data));
  }

  static getVersionsLevelHTML(): string {
    const data = this.mapedObjectData.versionLevel;
    return getInputFieldHTML(data.fieldListName, 'Version', generateConfigForHtml(data));
  }
  ...
  static getRemotesLevelHTML(): string {
    ...
    let resHTML = '';
    data.forEach(item => {
      resHTML += getInputFieldHTML(item.fieldListName, item.remoteName, generateConfigForHtml(item));
    });
    return resHTML;
  }
  ...
  static createDataListForVersionLevel() {
    const data = this.mapedObjectData.versionLevel;
    createNewDataList(data, this.checkCallBack.bind(this), () => {
    }, this.newTabIconEventClickedHandler.bind(this));
  }

  static createDataListForRemotesLevel() {
    const data = this.mapedObjectData.remotesLevel;
    data.forEach(item => {
      createNewDataList(item, this.checkCallBack.bind(this), () => {
      }, this.newTabIconEventClickedHandler.bind(this));
    });
  }
  ...
  static getRemotes(dataObj): IRemotesLevel[] {
    ...
    // todo: get remotes from snapshot
    const remotes = [];
  ...
  static validateCurrentValues(storageData: ICurrentValue) {
    ...
    const remotes = (tagVal as any)?.remotes;
    if (!remotes) {
      return;
    }
  ...
  static async setContent(accessToken: string) {
    ...
    // showLoginBtn({parent: this.mainContentBox, isLoading: !!accessToken});

    if (accessToken) {
      // await this.getAppData();
      // this.mainContentBox.innerHTML = getFormContent();
      // const currentFormValues = await session.currentFormValues;
      // if (currentFormValues) {
      //   this.validateCurrentValues(currentFormValues);
      // }
      this.initForm();
      this.getMainContent();
      return;
